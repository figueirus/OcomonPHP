  ...
  */session_start();
	...
	if ((!isset($_GET['action'])) and !isset($_POST['submit'])) {
        ...
        else
        {
                ...
                print "<TR class='header'><td class='line'>".TRANS('COL_MAJOR')."</TD><td class='line'>".TRANS('MNL_STATUS')."</TD><td class='line'><b>".TRANS('COL_EDIT')."</b></TD><td class='line'><b>".TRANS('COL_DEL')."</b></TD>";
                $j=2;
	...
	if ((isset($_GET['action'])  && $_GET['action']=="alter") && (!isset($_POST['submit']))) {
		...
		//<input type='text' class='text' name='data' value='".$rowAlter['data_feriado']."'>";
			...
			if ($rowAlter['status']==1) print " selected";
			print ">".TRANS('ACTIVE')."</option>";
			print"<option value=2";
			if ($rowAlter['status']==2) print " selected";
	...
	if ($_POST['submit'] = TRANS('BT_ALTER')){
		if ((!empty($_POST['descricao']))){
				...
				//$data = str_replace("-","/",$data);
				//$data = converte_dma_para_amd($data);
			$qry = "UPDATE reitorias set reit_nome='".noHtml($_POST['descricao'])."', status='".$_POST['status']."' where reit_cod=".$_POST['cod']."";
