<?php 
  ...
  */session_start();
	...
	if ((isset($_GET['action']) && ($_GET['action'] == "incluir") )&& empty($_POST['submit'])) {
		...
		print "</SELECT>";
		print "<br><input type='checkbox' name='areaadmin' value=1>".TRANS('COL_AREA_ADMIN')."<br>";
		// print "<input type='checkbox' name='userExterno' value=1>".TRANS('COL_USER_EXT')."";
	...
	if ((isset($_GET['action']) && $_GET['action']=="alter") && empty($_POST['submit'])) {
		...
		print "<tr>";
			...
			print "<br><input type='checkbox' name='areaadmin' value=1 ".$check.">".TRANS('COL_AREA_ADMIN')."<br>";
			// if ($row['user_externo']) {
				// $check = " checked";
			// } else $check = "";
			// print "<input type='checkbox' name='userExterno' value=1 ".$check.">".TRANS('COL_USER_EXT')."";
			print "</TD>";
		print "</TR>";
		print "<tr>";
...
/*			if ($_POST['areaadmin']){
			...
			if (isset($_POST['userExterno'])) {
				$userExterno = $_POST['userExterno'];
			} else
				$areaadmin = 0;

			// $qryins = "INSERT INTO usuarios (login, nome, password, data_inc, data_admis, email, fone, nivel, AREA, user_admin, user_externo) ".
			$qryins = "INSERT INTO usuarios (login, nome, password, data_inc, data_admis, email, fone, nivel, AREA, user_admin) ".
					...
					// .$_POST['area'].", '".$areaadmin."', '".$userExterno."')";
		...
		if (!$erro) {
			...
			if ($pass == $_POST['password2'])
					$query2 = "UPDATE usuarios SET nome='".noHtml($_POST['nome'])."', data_inc='".$data_inc."', ".
						...
						// "user_externo ='".$_POST['userExterno']."' WHERE user_id=".$_POST['login']."";
