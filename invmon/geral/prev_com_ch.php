...
//Equipamento que já tiveram preventiva
	...
	*/
	...
	if (isset($_GET['encadeado'])) {
        ...
        $QRY["full_detail_prev_ini"] = "SELECT c.comp_inv as etiqueta, c.comp_sn as serial, c.comp_nome as nome, c.comp_sn_os as sn_os, ".		//LRG
			...
			"\n\tc.comp_local as tipo_local, loc.loc_endereco,  ".
			...
			"\n\tassist.assist_desc as assistencia, ".
                        ...
                        "\n\tdate_add(oco.data_fechamento, interval cprev.conf_tempo_min day) as prev_min,". 
                        "\n\tdate_add(oco.data_fechamento, interval cprev.conf_tempo_max day) as prev_max".
...
/*			if ($negar=="NEG_SITUACAO") {
	...
	{
		print "<fieldset><legend>Máquinas com Preventivas</legend>";
		...
		$cont=0;
  		while ($row = mysql_fetch_array($resultado)) {
			...
			print "<td class='line'><a ".$alerta2." title='' href=\"javascript:monta_link('?prev_max=".$row['prev_max']."&ordena=fab_nome,modelo,local,etiqueta&coluna=modelo&ordenado=etiqueta','".$param."','NEG_SITUACAO')\">".formatDate($row['prev_max']). "</a></td>";
                        ...
                        print "<td class='line'><a title='Iniciar Chamado de Preventiva' href=\"../../ocomon/geral/incluir.php?invTag=".$row['etiqueta']."&problema=".$rowConf_prev['conf_num_chamado']."&descricao='Preventiva'&invInst=".$row['cod_inst']."&InvLoc=".$row['tipo_local']."&contato=".$_SESSION['s_usuario']."\"> <img src='".$iconPath."fone.png' alt='Incluir' /> </a></td>";
