...
is_file( "./includes/config.inc.php" )
	...
	if (!isset($_SESSION['s_intranet']))  $_SESSION['s_intranet']= 1;
	...
	$sql = "SELECT nome FROM usuarios WHERE login = '".$_SESSION['s_usuario']."' ";
	$exeSQL = mysql_query ($sql);
	$achou = mysql_fetch_array ($exeSQL);
	...
	if (empty($uLogado)) {
		...
		//Descreve o nível do usuário no Sistema
		if ($_SESSION['s_nivel'] < 3) {
			$USER_TYPE = TRANS('MNS_OPERADOR');
			if ($_SESSION['s_nivel'] == 1) {
				$USER_TYPE = TRANS('MNS_OPERADOR_ADMIN');
			} 
			if ($achou['flag_gestor'] == 1) {
				$USER_TYPE = TRANS('MNS_GESTOR');
			} 
		} else
...
############ PREVISÃO DO TEMPO ##############
$chave = 'fe1132f6'; // Obtenha sua chave de API gratuitamente em http://hgbrasil.com/weather
...
print "<table class='topo' border='0' id='cabecalho'>
	<tr>".
		//"<td ><img src='MAIN_LOGO.png' height='48' width='48'>&nbsp;&nbsp;<img src=tempo/imagens/".$dados['results']['img_id'].".png class='imagem-do-tempo' height='52' width='52'></td>".
		"<td ><img src='MAIN_LOGO.png' height='48' width='300'></td>".
		"<td align='center'>".$USER_TYPE.":<b> ".$uLogado."</b></td><td >|</td>".
		"<td ><a href='".$commonPath."logout.php' title='".$hnt."'>".$logInfo."&nbsp;<img src='includes/icons/logoff.png' style=\"vertical-align:middle;\" height='48' width='48' border='0'></a></td>".
		//"<td >&nbsp;&nbsp<img src=tempo/imagens/".$dados['results']['img_id'].".png class='imagem-do-tempo' height='52' width='72' align='center'>".$dados['results']['city']." &nbsp;&nbsp; ".$dados['results']['temp']." ºC</td>".
		"<td align='right'>".
		"<img src=tempo/imagens/".$dados['results']['img_id'].".png class='imagem-do-tempo' height='52' width='72' align='center'>".$dados['results']['city']." &nbsp;&nbsp; ".$dados['results']['temp']." ºC".
		//"<a href='http://www.twitter.com/ProjetoOcomon' target='_blank' title='Twitter: @ProjetoOcomon'>&nbsp;<img src='includes/icons/twitter.png' style=\"vertical-align:middle;\" height='48' width='48' border='0'></a>".
		"<a href='http://www.facebook.com/profile.php?id=100002304077585' target='_blank' title='Facebook: Projeto Ocomon Php'>&nbsp;<img src='includes/icons/facebook.png' style=\"vertical-align:middle;\" height='48' width='48' border='0'></a>".
		//"<a href='http://projetoocomon.blogspot.com' target='_blank' title='Blog Oficial Projeto Ocomon Php'>&nbsp;<img src='includes/icons/blog.png' style=\"vertical-align:middle;\" height='48' width='48' border='0'></a>".
		//"<a href='mailto:projeto.ocomonphp@gmail.com' title='Email de contato do Projeto Ocomon Php'>&nbsp;<img src='includes/icons/gmail.png' style=\"vertical-align:middle;\" height='48' width='48' border='0'></a> &nbsp;&nbsp;&nbsp; | &nbsp;".
		"<a onClick= \"javascript: popup('sobre.php')\" title='Sobre o sistema'>&nbsp;<img src='includes/icons/sobre.png' style=\"vertical-align:middle;\" height='48' width='48' border='0'></a></td>".
		"</td>
	</tr></table>";
print "<table class='barra' border='0px' id='barra'><tr>";
	...
	if (empty($_SESSION['s_permissoes'])&& $_SESSION['s_nivel']!=1){
		...
		#### MENU INTRANET
		//if ($_SESSION['s_intranet']=='1' && $achou['flag_gestor'] =='1' || $_SESSION['s_nivel']=='1' ){
		if ($achou['flag_gestor'] =='1' || $achouVincInst['flag_gestor'] == '1' || $_SESSION['s_nivel']=='1' || $_SESSION['s_nivel']=='2' ){
			print "<td id='INTRANET' width='7%'  class='barraMenu'><a class='barra' onMouseOver=\"destaca('INTRANET')\" onMouseOut=\"libera('INTRANET')\" onclick=\"loadIframe('menu.php?sis=intra','menu','','',3,'INTRANET')\">&nbsp;".TRANS('MNL_INTRA')."&nbsp;</a></td>"; 
			if ($sis=="") $sis="sis=intra";
			if ($sisPath=="") $sisPath=$intraDirPath;
			$sistem = "abertura.php";
			if ($marca=="") $marca = "INTRANET";
		} else
			print "<td width='7%' STYLE='border-right: thin solid #C7C8C6; color:#C7C8C6'>&nbsp;".TRANS('MNL_INTRA')."&nbsp;</td>";
		################
...
if ($_SESSION['s_logado']){
	...
	print "</tr>";
		...
		print "<form name='logar' method='post' action='".$commonPath."login.php?".session_id()."' onSubmit=\"return valida()\">";
			...
			print "<tr><td rowspan='4'><img src='includes/icons/encrypted.png' height='76' width='76' border='0'></a></td><td >".TRANS('MNS_USUARIO').":</td><td ><input type='text' class='help' name='login' value='".$typedUser."' id='idLogin' tabindex='1'></td><td rowspan='2'><input type='submit' class='blogin' value='".TRANS('cx_login')."' tabindex='3'></td></tr>". //class='help'
...
print "<tr><td colspan='2' align='center'><a href='http://projetoocomon.blogspot.com' target='_blank'>OcoMon</a> - ".TRANS('MNS_MSG_OCOMON').".<br>".TRANS('MNS_MSG_VERSAO').": ".VERSAO." - <b>ACF Tech - Soluções em  T.I - Desenvolvimento do Projeto Ocomon</b></td></tr>";
print "</table>";

//print "Nivel do usuário: ".$_SESSION['s_nivel'];
print "</body></html>";
...
var GLArray = new Array();
	function loadIframe(url1,iframeName1, url2,iframeName2,ACCESS,ID) {
		...
		var INTRA = document.getElementById('INTRANET');
