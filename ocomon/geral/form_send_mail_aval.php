...
*/session_start();
	...
	if (!isset($_POST['submit']) && !isset($_GET['action'])) {
		...
		print "<TR>";
			...
			print "<TD colspan='3' width='80%' align='left' bgcolor='".BODY_COLOR."'>";
					print "<INPUT type='text' class='select_sol' name='field_subject' id='idFieldSubject' value='T.I - UNILASALLE - Avaliação do ".TRANS('OCO_CALL').": ".$numero."'>";
		...
		print "</TR>";
			...
			print "<tr><TD width='20%' bgcolor='".TD_COLOR."' >".TRANS('MAIL_FIELD_BODY').":&nbsp;".
				...
				print "<TEXTAREA class='textarea' name='field_body' id='idFieldBody'>Prezado(a) %contato%,
O chamado %numero% foi encerrado no sistema de atendimento.
Para melhor atende-lo(a), gostariamos da sua avaliação para este chamado.
Clique no link abaixo para deixar a sua avaliação e comentário para este atendimento.
%site2%

Atte,
Equipe de Tecnologia da Informação - Unilasalle Canoas</textarea>";
...
/*				?>
	...
	if (isset($_POST['submit']) && $_POST['submit'] == TRANS("BT_SEND")){
		...
		//$VARS['%site2%'] = $_SESSION['s_ocomon_site']."/ocomon/geral/qualidade.php?numero=".$_POST['numero']."";
		$VARS['%site2%'] = "<a href='".$_SESSION['s_ocomon_site']."/ocomon/geral/qualidade.php?numero=".$_POST['numero']."'>Clique aqui para avaliar agora o seu chamado - Suporte ao Usuário - T.I</a>";
		...
		print ">mensagem('".$msg."'); window.opener.location.reload(); window.close();</script>";
		//print "<script language='javaScript'>alert('".$msg."');window.close();</script>";
	}
