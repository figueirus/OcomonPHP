  ...
  */session_start();
	...
	if (!isset($_POST['submit'])) {
		...
		print "</TR>";
			...
			$query_todas="select * from instituicao order by inst_nome";
			...
			print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'>";
				...
				print "<SELECT  class='select' name='local' id='idLocal' size=1>";
	        		        print "<option value= '-1'>".TRANS('SEL_SECTOR')."</option>";
	                		 $query = "SELECT l .  * , pr.prior_nivel AS prioridade, staLoc.status_desc, staLoc.status_id as status
			...
			print "<tr><td bgcolor='".TD_COLOR."'>Forma de pagamento:</td>";
				print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'><select class='select' name='forma_pagto' id='idFormaPagto'>";
					print "<option value='-1'>Selecione a forma de pagamento</option>";
							$sql="select * from pagto_forma order by id_forma";
							$commit = mysql_query($sql);
							while($rowOS = mysql_fetch_array($commit)){
								print "<option value=".$rowOS['forma_pagto'].">".$rowOS["forma_pagto"]."</option>";
	...
	if (isset($_POST['submit']) && $_POST['submit']== TRANS('BT_OK') ) {
		...
		##################### INSERINDO DADOS NA TABELA PAGAMENTOS ######################
			...
			."\n VALUES ('".$_POST['numero']."','".$_POST['local']."','".$dataBanco."' "
		...
		if ( (FDate($data_agendamento) == '0000-00-00 00:00:00') ){
			...
			############################################################################
			### PREENCHIMENTO AUTOMÁTICO DE ALGUNS CAMPOS EDITADOS
			############################################################################
			if ($_SESSION['s_formatBarOco']) {
			...
			if ($row['status'] != $_POST['status']){
				...
				if ($_POST['chk_squedule']!=""){
					$sql = "SELECT conf_schedule_status_2 FROM config";
					$exec = mysql_query($sql);
					$achou = mysql_fetch_array($exec);

					$query1 = "SELECT status from status WHERE stat_id = ".$achou['conf_schedule_status_2']."";
					$resultado1 = mysql_query($query1);
					$row1 = mysql_fetch_array($resultado1);
			...
			if ($row['contato'] != $_POST['contato']){
				$assentamento .= "<b>".TRANS('OCO_FIELD_CONTACT')."</b>: ".$row['contato']." --> ".$_POST['contato']."<br />";
			}
				...
				if ($gravaImg[$i]) {
					...
					if(chop($fileinput)!=""){
						...
						$SQL = "Insert Into imagens (img_nome, img_oco, img_tipo, img_bin, img_largura, img_altura, img_size) values ".
								...
								"('".$_FILES[$nomeAnexo]['name']."',".$_POST['numero'].", '".$_FILES[$nomeAnexo]['type']."', ".
			...
			{   //$status!=1 and
				if (($_POST['data_atend']=="") and ($depois!=4) and (isset($_POST['resposta'])) ) //para verificar se já foi setada a data do inicio do atendimento. //Se eu incluir um assentamento seto a data de atendimento
				{
					$query = "UPDATE ocorrencias SET operador=".$_POST['operador'].", problema = ".$catProb.", instituicao='".$_POST['institui']."', equipamento = '".$_POST['etiq']."', sistema = '".$_POST['sistema']."', local=".$_POST['local'].", data_fechamento=NULL, status=".$depois.", data_atendimento='".date('Y-m-d H:i:s')."', ".
								"data_abertura = '".$data_agendamento."', oco_real_open_date='".$realOpenDate."', oco_scheduled=".$agendado.", descricao='".$description."', contato='".$_POST['contato']."', telefone='".$_POST['ramal']."', oco_prior='".$_POST['prioridade']."' WHERE numero=".$_POST['numero']."";
