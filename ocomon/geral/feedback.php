...
<?php 	
	...
	$linhas2 = mysql_num_rows($resultado2);
		...
		if ($s_nivel == 1) $linkEdita = "<br><b><a href='altera_dados_ocorrencia.php?numero=".$numero."'>Editar ocorrência como admin:</a></b><br>"; else
			$linkEdita = "<b>Dar feedback para o chamado</b>";			
...
<?php 
	print "<div id='div' style='display:none'>";
...
?>
        ...
        if ($linhas2!=0)
        {
				...
				while ($rowAS=mysql_fetch_array($resultado2))
                {
                        ?>
                        <tr>        
								<TD align="left" bgcolor="<?php print TD_COLOR?>" valign="top">Assentamento <?php print $i+1;?> de <?php print $linhas2;?> por <b><?php print $rowAS['nome'];?></b> em <?php print datab($rowAS['data']);?></TD>
        ...
        <TR>
                ...
                <TD colspan='5' width="80%" align="left" bgcolor=<?php print BODY_COLOR?>><TEXTAREA id="idAssentamento" class="textarea"  name="assentamento">Digite aqui o seu feedback para este chamado</textarea> 
						<?php 
                            //if ($data_atend =="") { 
		...
		<?php 
			...
			print "<tr><td bgcolor='".TD_COLOR."'>Enviar e-mail para:</td>".
					"<td colspan='2'><input type='checkbox' value='ok' name='mailAR' title='Envia email para a área selecionada para esse chamado'>&Aacute;rea Respons&aacute;vel&nbsp;&nbsp;".
...
/*								$flag = mail_user_assentamento($rowMailUs['email'], $rowresposta['sis_email'], $rowMailUs['nome'],$_GET['numero'],
							...
							$resultado3 = mysql_query($queryA) or die('N&Atilde;O FOI POSS&Iacute;VEL GRAVAR AS INFORMA&Ccedil;&Otilde;ES DE EDI&Ccedil;&Atilde;O DO CHAMADO!<br>'.$queryA);
								
                            	if ($antes != $depois) //Status alterado!!
				{   //$status!=1 and 
					
					
					// print "Operador: ".$operador."<br>";
					// print "Problema: ".$problema."<br>";
					// print "Inst.: ".$institui."<br>";
					// print "Sistema: ".$sistema."<br>";
					// print "Local: ".$local."<br>";
					// print "Resposta: ".$resposta."<br>";
					// print "Etiqe: ".$etiq."<br>";
					// print "Status: ".$status."<br>";
					// print "Data_Atend: ".$data_atend."<br>";
					// print "Etiqe: ".$status."<br>";
					// exit;
					...
					$aviso = "Ocorrencia alterada com sucesso! ";
