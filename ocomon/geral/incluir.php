...
session_start();
	...
	######## QUALIFICAÇÃO ######## 
	$Sqlqualit = "SELECT * FROM ocorrencias o INNER JOIN usuarios u on (u.user_id = o.aberto_por) where o.usu_nota = 7  and u.nivel not in ('1','2') and u.user_id=".$_SESSION['s_uid']." and o.data_fechamento >'2010-06-15 00:00:00'";
	//$Sqlqualit = "SELECT * FROM ocorrencias o INNER JOIN usuarios u on (u.user_id = o.aberto_por) where o.usu_nota = 0 and u.user_id=".$_SESSION['s_uid']."";
	$execqualit = mysql_query($Sqlqualit) or die (TRANS('ERR_QUERY'));
	$contqualit = mysql_num_rows ($execqualit);
	if ($contqualit>1 and (date(Ymd) <'20120601'))
	//if ($contqualit>1)
	{
		print "<script>mensagem('".TRANS('OCO_AVISOQUALIT')."'); window.close();</script>";
	}
	if ($contqualit>2 and (date(Ymd) >='20120601'))
	//if ($contqualit>=2)
	{	
		print "<script>mensagem('".TRANS('OCO_AVISOQUALIT2')."'); redirect('abertura_user.php?action=listall');</script>";
	exit;
	}
...
##################################################
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_desc']) || empty($rowconf)) {
			...
			if (!$_SESSION['s_formatBarOco']) {
				print "<TEXTAREA class='textarea' name='descricao' id='idDescricao'  >".$descricao."</textarea>"; //onChange=\"Habilitar();\"
			} else {
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_unit']) || empty($rowconf)) {
			...
			### VERIFICA VINCULAÇÃO COM INSTITUIÇÃO ##
			$sqlVinc = "SELECT DISTINCT inst.*
					FROM usuarioXinstituicao vinc, instituicao inst, usuarios user
					WHERE vinc.inst_cod = inst.inst_cod
					AND vinc.user_id = ".$_SESSION['s_uid']."";
			$execVinc = mysql_query($sqlVinc);
						
			$query2 = "SELECT * from instituicao WHERE inst_status not in (0) order by inst_nome";
			...
			if (isset($_POST['instituicao'])){
				...
				### VINCULAÇÃO USER SOMENTE ABERTURA
				if ($_SESSION['s_nivel'] == 3){
					while($rowVinc = mysql_fetch_array($execVinc)){
						print "<option value=".$rowVinc['inst_cod']."";
							if ($rowVinc['inst_cod']== $invInst) print " selected";
						print ">".$rowVinc['inst_nome']."</option>";	
					}
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_tag']) || empty($rowconf)) {
			...
			print "&nbsp;".TRANS('OCO_FIELD_OF_EQUIP','do equipamento').":</TD>";
            		...
            		print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'><INPUT type='text' class='text4' name='equipamento' id='idEtiqueta' value='".$invTag."' >";//onChange=\"Habilitar();\"
				...
				// print "<tr id='tr_anexo_$i' $estilo>";
					...
					if($i != $row_config['conf_qtd_max_anexos']){
						...
						print "<a id='link_adic_$i'
									onclick=\"
									javascript:document.getElementById('tr_anexo_".($i+1)."').style.display='';
		...
		if ((!empty($rowconf) && $rowconf['conf_scr_mail']) || empty($rowconf)) {
			print "<td bgcolor='".TD_COLOR."'>".TRANS('OCO_FIELD_SEND_MAIL_TO','Enviar e-mail para').":</td>".
				"<td colspan='2'><input type='checkbox' value='ok' name='mailAR' checked>".TRANS('OCO_FIELD_AREA','')."&nbsp;&nbsp;".
								"<input type='checkbox' value='ok' name='mailCLI' checked>Cliente&nbsp;&nbsp;".
		...
		if (isset($_POST['OK'])==TRANS('BT_OK')) {
			...
			while ($i<=$replicate) //'".noHtml($descricao)."'
			{
				...
				#### VERIFICA SE O USUÁRIO É EXTERNO OU NÃO ####
				$sqlUserExt = "SELECT user_externo FROM usuarios WHERE user_id = '".$_SESSION['s_uid']."' ";
				$execUserExt = mysql_query($sqlUserExt);
				$achouUserExt = mysql_fetch_array($execUserExt);
				
				if ($achouUserExt['user_externo'] == 1){
					$sistema = $_POST['area'];
					$catProb = $_POST['problema'];
					// $catProb = "REDEBILA ".$problema;
				...
				##### VERIFICA SE ETIQUETA ESTA VAZIA #####
				...
				if (!$schedule){
					...
					$query.="".$_POST['instituicao'].",'".$equipamento."','".$sistema."',".
					"'".$_POST['contato']."','".$_POST['telefone']."',".$_POST['local'].",".$operator.",".
			...
			if ($resultado == 0) {
				...
				$VARS['%linkglobal%'] = "<a href='".$row_config['conf_ocomon_site']."/ocomon/geral/mostraConsultaExterno.php?numero=".$numero."&id=".$globalID."'>".$row_config['conf_ocomon_site']."</a>";
				...
				$VARS['%data_abertura%'] = formatDate(date("Y-m-d H:i:s"));
				$VARS['%solucao%'] = '';
				$VARS['%versao%'] = VERSAO;
				...
				if (isset($_POST['mailCLI'])) {
					...
					$sqlCliente = "SELECT * FROM localizacao WHERE loc_id = '".$rowfull['loc_id']."' ";
					$execCliente =  mysql_query($sqlCliente) or die(TRANS('ERR_QUERY'));
					$linhaCliente = mysql_fetch_array($execCliente);
					...
					// send_mail($event, $linhaCliente['loc_email'], $rowconfmail, $rowmsg, $VARS);
				}
				#################################################
...
<!--
	...
	function valida(){
		...
		if (!LOAD) {
			...
			var tipoAtend = document.getElementById('id_atend');
