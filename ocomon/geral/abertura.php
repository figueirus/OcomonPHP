...
// print "A página demorou {$tempo_carregamento} segundos para carregar."; 
	...
	$resultado = mysql_query($query) or die (TRANS('ERR_QUERY').$query);
		...
		while ($resposta = mysql_fetch_array($resultado))
		{
			...
			print "<TD class='line'>".$resposta['avisos']."</TD>";
			print "<TD class='line'>".$resposta['nome']."</TD>";
	...
	if ($linhas == 0) {
		...
		print "<TR class='header'>";
			print "<TD  class='line' >".TRANS('OCO_NUMBER_BRIEF')."</TD><TD class='line' >".TRANS('OCO_PROB')."<BR>".TRANS('COL_TAG')."</TD>".
		...
		if ($varAux == ''){
			$unidate = "<b>".$rowAT['unidade']." - Sem informação </b>";
		}else{
			if ($rowAT['etiqueta'] == '0'){
				$link = "<font color=red><b>".$rowAT['unidade']." - Sem informação </b></font>";
			}else{
				$unidade = $varAux[0]." - ".str_pad($rowAT['etiqueta'],6,"0",STR_PAD_LEFT);
				//$unidade = $rowAT['unidade']." - ".$rowAT['etiqueta'];
				$link = "<a onClick= \"javascript: popup_alerta('mostra_oco.php?popup=true&comp_inv=".$rowAT['etiqueta']."&comp_inst=".$instituicao."')\" title='".TRANS('HNT_OCCO_EQUIP')."'>".$unidade."</a>";
			}
		}
		##############################
		
		print "<TD  class='line'  ".$valign."><a href='mostra_consulta.php?numero=".$rowAT['numero']."'>".$rowAT['numero']."</a>".$imgSub."</TD>";
		print "<TD  class='line'  ".$valign.">".$linkImg."&nbsp;".$rowAT['problema']."<BR><b>".$link."</b></TD>";
...
//
		...
		print "<TR class='header'>";
			print "<TD  class='line' >".TRANS('OCO_NUMBER_BRIEF')."</TD><TD class='line' >".TRANS('OCO_PROB','Problema')."</TD>".
				"<TD  class='line' >".TRANS('OCO_CONTACT')."<BR>".TRANS('OCO_PHONE','Ramal')."/".TRANS('COL_UNIT')."</TD>".
				"<TD  class='line'  WIDTH='250'>".TRANS('OCO_LOCAL')."</TD>".
				"<TD  class='line' >".TRANS('OCO_STATUS')."</TD>".
		...
		if ($varAux == ''){
			...
			if ($rowAT['etiqueta'] == '0'){
				//$link = "<font color=red>Etiqueta não informada</font>";
				$link = "<b>".$rowAT['unidade']."<font color=red> - Sem informação </b></font>";
			}else{
				//$unidade = $varAux[0]." - ".$rowAT['etiqueta'];
				$unidade = $rowAT['unidade']." - ".str_pad($rowAT['etiqueta'],6,"0",STR_PAD_LEFT);
				$link = "<a onClick= \"javascript: popup_alerta('mostra_oco.php?popup=true&comp_inv=".$rowAT['etiqueta']."&comp_inst=".$instituicao."')\" title='".TRANS('HNT_OCCO_EQUIP')."'>".$unidade."</a>";
			}
		}
		#####################
		...
		print "<TD  class='line'  ".$valign."><b>".$rowAT['contato']."</b><br>".$rowAT['telefone']."/ <b>".$link."</b></TD>";
		print "<TD  class='line'  ".$valign."><b>".$rowAT['setor']."</b><br>";
		$texto = trim($rowAT['descricao']);
		if (strlen($texto)>200){
			$texto = substr($texto,0,195)." ..... ";
...
// 	$PAGE->execSQL();
	...
	//FILA DE CHAMADOS
        ...
        print "<TD  class='line'  nowrap>".TRANS('OCO_NUMBER_BRIEF','N.º')."&nbsp;/&nbsp;<a onClick=\"redirect('".$_SERVER['PHP_SELF']."?&ORDERBY=AREA')\" title='Ordena por Área de atendimento'>".TRANS('OCO_AREA','Área')."".$ICON_ORDER_AREA."</a></TD>".
        	"<TD  class='line' ><a onClick=\"redirect('".$_SERVER['PHP_SELF']."?&ORDERBY=PROB')\" title='Ordena por tipo de problema'>".TRANS('OCO_PROB')."".$ICON_ORDER_PROB."</a><BR>".TRANS('COL_TAG')."</TD>".
		...
		if ($varAux == ''){
			$unidate = "<b>".$row['unidade']." - Sem informação </b>";
		}else{
			if ($row['etiqueta'] == '0'){
				//$link = "<font color=red>Etiqueta não informada</font>";
				$link = "<b>".$row['unidade']."<font color=red> - Sem etiqueta </b></font>";
			}else{
				//$dado[$i]['ALU_ID'] = "1".$this->escola.trim(str_pad($parte[0], 8, "0", STR_PAD_LEFT));
				$unidade = $varAux[0]." - ".str_pad($row['etiqueta'],6,"0",STR_PAD_LEFT);
				//$unidade = $row['unidade']." - ".$row['etiqueta'];
				$link = "<a onClick= \"javascript: popup_alerta('mostra_oco.php?popup=true&comp_inv=".$row['etiqueta']."&comp_inst=".$instituicao."')\" title='".TRANS('HNT_OCCO_EQUIP')."'>".$unidade."</a>";
			}
		}
		#######################
		print "<TD  class='line'  ".$valign."><b><a href='mostra_consulta.php?numero=".$row['numero']."'>".$row['numero']."</a></b>".$imgSub."<br>".($row['area']==''?'&nbsp;':$row['area'])."</td>";
		//print "<TD  class='line'  ".$valign.">".$linkImg."&nbsp;".($row['problema']==''?'&nbsp;':$row['problema'])."<BR><b>".$unidade	."</b></td>";
		print "<TD  class='line'  ".$valign.">".$linkImg."&nbsp;".$row['problema']."<BR><b>".$link."</b></TD>";
		...
		if ($sep[0]>20) { //Se o chamado estiver aberto a mais de 20 dias o tempo é mostrado em dias para não ficar muito pesado.
			...
			$imgSlaS = 'checked.png';
				if ($row['status_cod'] == 16) {
					...
					"<br><font color='#5D478B'><b>".$row['chamado_status']."</b></font></TD>";
				}else{
					if ($row['status_cod'] == 1){
						print "<TD  class='line'  ".$valign."><font color='red'><a onClick=\"javascript:popup('mostra_hist_status.php?popup=true&numero=".$row['numero']."')\">".$sep[0]." dias</a></font>".
						"<br><font color = red><b>".$row['chamado_status']."</b></font></TD>";
					}else{
						print "<TD  class='line'  ".$valign."><font color='red'><a onClick=\"javascript:popup('mostra_hist_status.php?popup=true&numero=".$row['numero']."')\">".$sep[0]." dias</a></font>".
			...
			//DESTAQUE PARA OS CHAMADOS QUE ESTIVEREM AGUARDANDO ATENDIMENTO.
			if($row['chamado_status'] == "Aguardando atendimento"){
				$statusChamado = "<font color=red><b>Aguardando atendimento</b></font>";
			}elseif($row['chamado_status'] == "Aguardando feedback do usuário"){
					$statusChamado = "<font color='#5D478B'><b>Aguardando feedback do usuário</b></font>";
				}elseif($row['chamado_status'] == "Aguardando retorno do fornecedor"){
						$statusChamado = "<font color='cian'><b>Aguardando retorno do fornecedor</b></font>";
				}elseif($row['chamado_status'] == "Aguardando retirada do cliente"){
						$statusChamado = "<font color='#993399'><b>Aguardando retirada do cliente</b></font>";
					}else{
						$statusChamado = $row['chamado_status'];
					}
