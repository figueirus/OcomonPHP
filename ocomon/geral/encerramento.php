<?php 
  ...
  */session_start();
	...
	print "<HTML>";
	print "<meta http-equiv='Content-Type' content='text/html; charset=iso-8859-1'/><BODY bgcolor='".BODY_COLOR."' ".
...
################################################################
		...
		print "<TR>";
			...
			print "<TD width='30%' align='left' bgcolor='".BODY_COLOR."'>";
				print "<select class='select' name='loc' id='idLocal'>";
					...
					$qrylocal = "SELECT l .  * , pr.prior_nivel AS prioridade, staLoc.status_desc, staLoc.status_id as status
					...
					while($row_loc=mysql_fetch_array($exec_local)){
						if ($row_loc['loc_status'] == '1'){
							$class = "class='option2'";
						}else{
							$class = "class='option'";
						}
						print "<option ".$class." value=".$row_loc['loc_id']."";
							if ($row_loc['loc_id'] == $loc){
								print " selected";
							}
		...
		print "<TABLE border='0'  width='100%'>";
			########## VALOR O.S ############
			print "<tr><td bgcolor='".TD_COLOR."'>Forma de pagamento:</td>";
				print "<TD width='80%' align='left' bgcolor='".BODY_COLOR."'><select class='select' name='forma_pagto' id='idFormaPagto'>";
					print "<option value='-1'>Selecione a forma de pagamento</option>";
							$sql="select * from pagto_forma order by forma_pagto";
							$commit = mysql_query($sql);
							while($row = mysql_fetch_array($commit)){
								print "<option value=".$row['id_forma'].">".$row["forma_pagto"]."</option>";

							} // while
						print "</select>";
				print "</td>";
			print "</tr>";
			print "<tr><td bgcolor='".TD_COLOR."'>".TRANS('VALOR_OS').":</td>";
				// print "<TD colspan='5' width='80%' align='left' bgcolor='".BODY_COLOR."'>";
					print "<TD width='80%' align='left' bgcolor=".BODY_COLOR."><INPUT type='text' name='pagto' class='text' id='idPagto' onKeydown=\"Formata(this,20,event,2);\"></TD>";
				print "</TD>";
			print "</tr>";
			##################################
			print "<tr><td bgcolor='".TD_COLOR."'>".TRANS('OCO_FIELD_SEND_MAIL_TO').":</td>".
					"<td colspan='3' width='360px' ><input type='checkbox' value='ok' name='mailAR' checked title='".TRANS('MSG_SEND_EMAIL_AREA_ATTEND_CALL')."'>".TRANS('OCO_FIELD_AREA')."&nbsp;&nbsp;".
	...
	if (isset($_POST['submit'])) {
		...
		##################### INSERINDO DADOS NA TABELA PAGAMENTOS ######################
			...
			// $valor = number_format($_POST['pagto'],2,'.',',');
			// $valor = str_replace(",",".",$_POST['pagto']);
			$valor = str_replace(',', '.', str_replace('.', '', $_POST['pagto']));

			$dataBanco = date('Y-m-d H:i:s');
			$sqlPagto = "INSERT INTO pagto_chamado (ocorrencia, cliente, data, valor, tipo_pagto) "
			."\n VALUES ('".$_POST['numero']."','".$_POST['loc']."','".$dataBanco."' "
			."\n ,'". $valor."', '".$_POST['forma_pagto']."'  )";
			
			// var_dump ($sqlPagto); exit;
			
			$result = mysql_query($sqlPagto);

		#################################################################################
					
		
		### Verifica se o problema SUSPEITA DE HARDWARE foi alterado
		if ($_POST['radio_prob'] == '349'){
			print "<script>mensagem('".TRANS('MSG_CALL')."');javascript:history.back()</script>";
		}else{
		...
		##########################################################################################
			...
			if ($atendimento==null) {
				$query2 = "UPDATE ocorrencias SET status=".$status.", local=".$_POST['loc'].", problema ='".$radio_prob."', ".
					...
					"contato='".$_POST['contato']."', data_fechamento='".date('Y-m-d H:i:s')."', ".
					"data_atendimento='".date('Y-m-d H:i:s')."', oco_script_sol=".$_POST['script_sol']." WHERE numero='".$_POST['numero']."'";

			} else {
				$query2 = "UPDATE ocorrencias SET status=".$status.", local=".$_POST['loc'].",problema ='".$radio_prob."', ".
					"operador=".$_SESSION['s_uid'].", instituicao='".$_POST['inst']."', equipamento='".$_POST['etiqueta']."', ".
					"contato='".$_POST['contato']."', data_fechamento='".date('Y-m-d H:i:s')."', oco_script_sol=".$_POST['script_sol']." ".
			...
			if (($resultado == 0) or ($resultado1 == 0) or ($resultado2 == 0))
			{
				...
				$VARS['%site2%'] = "<a href='".$_SESSION['s_ocomon_site']."/ocomon/geral/qualidade.php?numero=".$rowfull['numero']."'>".TRANS('TXT_AVAL')."</a>";
				$VARS['%area%'] = $rowfull['area'];
				$VARS['%operador%'] = $rowfull['nome'];
				$VARS['%data_abertura%'] = formatDate($rowfull['data_abertura']);
				$VARS['%data_fechamento%'] = formatDate($rowfull['data_fechamento']);
				$VARS['%problema%'] = $rowfull['problema'];
				$VARS['%desc_problema%'] = $_POST['problema'];
				...
				if (isset($_POST['mailCLI'])) {
					...
					$event = 'email-encerrado-cli';
					$qrymsg = "SELECT * FROM msgconfig WHERE msg_event like ('".$event."')";
					$execmsg = mysql_query($qrymsg) or die(TRANS('ERR_QUERY'));
					$rowmsg = mysql_fetch_array($execmsg);

					// send_mail($event, $linhaCliente['loc_email'], $rowconf, $rowmsg, $VARS);
				}
				## ENVIO DE EMAIL DE AVALIAÇÃO
				if (isset($_POST['mailCLI'])) {
					$sqlCliente = "SELECT * FROM localizacao WHERE loc_id = '".$rowfull['loc_id']."' ";
					$execCliente =  mysql_query($sqlCliente) or die(TRANS('ERR_QUERY'));
					$linhaCliente = mysql_fetch_array($execCliente);
					
					$event = 'email-avaliacao';
					$qrymsg = "SELECT * FROM msgconfig WHERE msg_event like ('".$event."')";
					$execmsg = mysql_query($qrymsg) or die(TRANS('ERR_QUERY'));
					$rowmsg = mysql_fetch_array($execmsg);
...
<!--
	...
	function valida(){
		...
		if (ok) var ok = validaForm('idDesc','','Descriçãoo técnica',1);
		if (ok) var ok = validaForm('idSolucao','','Solução',1);
		if (ok) var ok = validaForm('idNotaAval','COMBO','campo nota',1);
