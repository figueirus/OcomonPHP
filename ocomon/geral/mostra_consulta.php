<?php session_start();
	...
	###### QUALIFICAÇÃO DE CHAMADOS #####
	if ($row['status_cod'] == 16 && $_SESSION['s_nivel'] == 3) {
			print "<TD align='right' width='25%'><a href=feedback.php?numero=".$row['numero']."><font color=red>".TRANS('FEEDBACK_CHAMADO')."</font></a>&nbsp;|&nbsp;</TD>";//&nbsp;|&nbsp;
	}
	//print $_SESSION['s_nivel'];
	if ($row['status_cod'] == 4 && $_SESSION['s_nivel'] == 3 && $row['usu_nota'] == 7) {
			print "<TD align='right' width='25%'><a href=qualidade.php?numero=".$row['numero']."><font color = red>".TRANS('QUALIT_CHAMADO')."</font></a>&nbsp;|&nbsp;</TD>";//&nbsp;|&nbsp;
	}	
	###############################

	#### IMPRESSAO DE RECIBO #####
	print "<TD align='right' width='10%' ><a href='impressaoRecibo.php?numero=".$row['numero']."' target='_blank'>Imprimir Recibo</a>&nbsp;|&nbsp;</TD>"; //&nbsp;|&nbsp;
	print "<TD align='right' width='10%' ><a onClick=\"javascript:popup_alertaII('mostraCheckList.php?popup=true&numero=".$row['numero']."')\"></TD>"; //&nbsp;|&nbsp;
	if ($_SESSION['s_nivel'] < 3)
	...
	print "</TR>";
        ...
        if ($row['status_cod'] == 4)
	{
		$sql = "SELECT * FROM nota_aval WHERE nota_aval_id = ".$row['usu_nota']."";
		$exec = mysql_query($sql);
		$linha = mysql_fetch_array($exec);
		$nota = $linha['nota_aval'];
		...
		if ($row['data_abertura'] != $row['oco_real_open_date'] && $row['oco_real_open_date']!='0000-00-00 00:00:00'){
			...
			##### AVALIAÇÃO DO CHAMADO #####
				print "<TD width='20%' align='left' bgcolor='".TD_COLOR."'>".TRANS('OCO_NOTA')."</TD>";
				if(($linha['nota_aval_id'] == 4) OR ($linha['nota_aval_id']  == 5) ){
					print "<TD  align='left' ><font color=red><b>".$nota."</b></font></TD>";
				}else{
					if(($linha['nota_aval_id'] == 1) OR ($linha['nota_aval_id']  == 2) OR ($linha['nota_aval_id']  == 3) ){
						print "<TD  align='left' ><font color=green><b>".$nota."</b></font></TD>";
					}else{
						if ($linha['nota_aval_id'] == 7){
							$nota = "Chamado não qualificado";
							print "<TD  align='left' ><font color=red><b>".$nota."</b></font></TD>";
						}else{
							print "<TD  align='left' ><b>".$nota."</b></TD>";
						}
					}
				}
		} else {
		...
		########## VALOR O.S ############
		$sqlPagto = "SELECT pg.valor, pg.tipo_pagto, f.forma_pagto 
				FROM pagto_chamado AS pg, pagto_forma AS f
				WHERE ocorrencia = '".$row['numero']."'
				AND pg.tipo_pagto = f.id_forma  ";
		$execPagto = mysql_query($sqlPagto);
		$linha = mysql_fetch_array ($execPagto);
		$valor = number_format($linha['valor'], 2, ',', '.');
		
		print "<tr><td bgcolor='".TD_COLOR."'><b>".TRANS('VALOR_OS').":</b></td>";
			print "<TD colspan='5' width='80%' align='left' bgcolor='".BODY_COLOR."'><input class='disable' value='".$valor."' disabled></TD>";
		print "</tr>";		
		print "<tr><td bgcolor='".TD_COLOR."'><b>Forma de Pagamento:</b></td>";
			print "<TD colspan='5' width='80%' align='left' bgcolor='".BODY_COLOR."'><input class='disable' value='".$linha['forma_pagto']."' ></TD>";
		print "</tr>";
		##################################
